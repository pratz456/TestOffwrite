...
export default function DashboardScreen({
  ...
  const taxInputKey = JSON.stringify({
    ...
    taxRetry,
    profile: {
      revision: recordRevision(profile?.updated_at ?? profile?.updatedAt),
      filingStatus: profile?.filing_status ?? profile?.filingStatus ?? null,
      state: profile?.state ?? null,
      mailingAddress: profile?.mailing_address ?? null,
      income: profile?.income ?? null,
      businessIncome: profile?.business_income ?? profile?.businessIncome ?? null,
      w2Income: profile?.w2_income ?? profile?.w2Income ?? null,
      w2SocialSecurityWages: profile?.w2_social_security_wages ?? null,
      w2MedicareWages: profile?.w2_medicare_wages ?? null,
      w2FederalWithheld: profile?.w2_federal_withheld ?? null,
      homeOfficeMethod: profile?.home_office_method ?? null,
      healthInsurancePremiums: profile?.health_insurance_premiums ?? null,
      healthInsurancePremium: profile?.health_insurance_premium ?? null,
      sepIraContribution: profile?.sep_ira_contribution ?? null,
      solo401kContribution: profile?.solo_401k_contribution ?? null,
      simpleIraContribution: profile?.simple_ira_contribution ?? null,
      retirementContribution: profile?.retirement_contribution ?? null,
      hsaContribution: profile?.hsa_contribution ?? null,
      priorYearTax: profile?.prior_year_tax ?? null,
    },
