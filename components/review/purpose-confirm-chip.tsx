 ...
 */
export function PurposeConfirmChip({ proposal, question, busy = false, disabled = false, onConfirm, onReject }: PurposeConfirmChipProps) {
  ...
  return (
    <section className="space-y-2 rounded-xl border border-primary/20 bg-primary/5 p-3" aria-labelledby="purpose-proposal-heading">
      <p id="purpose-proposal-heading" className="flex items-start gap-1.5 text-xs font-medium text-primary">
        <Sparkles aria-hidden="true" className="mt-0.5 h-3.5 w-3.5 shrink-0" />
        <span>{question?.trim() || 'Proposed business purpose'}</span>
      </p>
      {editing ? (
        <div className="space-y-2">
          <label htmlFor="purpose-proposal-edit" className="sr-only">Business purpose</label>
          <textarea id="purpose-proposal-edit" value={draft} onChange={event => setDraft(event.target.value)} maxLength={500} rows={2} disabled={blocked}
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <button type="button" disabled={blocked || !trimmed} onClick={() => onConfirm(trimmed)}
              ...
              {busy ? <Loader2 aria-hidden="true" className="h-4 w-4 animate-spin" /> : <Check aria-hidden="true" className="h-4 w-4" />}{busy ? 'Saving…' : 'Confirm purpose'}
            </button>
          </div>
        </div>
      ) : (
        <div className="flex flex-wrap gap-2">
          <button type="button" disabled={blocked} onClick={() => onConfirm(proposal!)} aria-label={`Confirm purpose: ${proposal}`}
            ...
            <span className="min-w-0"><span className="block text-xs font-normal opacity-90">Confirm purpose</span><span className="block break-words">{proposal}</span></span>
          </button>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-xs leading-4 text-muted-foreground">Confirming saves this purpose and records the deduction. Nothing is saved until you tap.</p>
