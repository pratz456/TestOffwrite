...
export function QuarterlyEstimateClient() {
  ...
  return <div className="min-h-screen bg-background"><LandingHeader /><main className="mx-auto max-w-6xl space-y-4 px-4 py-6 sm:px-6 lg:px-8">
    <header><h1 className="text-2xl sm:text-3xl font-semibold tracking-tight">Quarterly Payment Planning</h1><p className="mt-2 max-w-3xl text-sm leading-relaxed text-muted-foreground">Compare ordinary federal estimated-payment methods using a reviewed annual tax forecast. This is a planning estimate: it does not calculate your income tax return or decide what you should pay today.</p></header>
    <div className="grid items-start gap-4 lg:grid-cols-2"><Card><CardHeader><CardTitle>Review your inputs</CardTitle></CardHeader><CardContent>
      <form onSubmit={calculate} className="space-y-3 text-sm [&_input:not([type=checkbox])]:min-h-11 [&_select]:min-h-11">
        ...
        {form.priorAvailability === 'eligible' && <>
          ...
          <label className="flex min-h-11 items-start gap-2 rounded-lg border border-border p-3 text-sm leading-relaxed [&_input]:mt-1"><input type="checkbox" checked={form.priorEligible} onChange={e => change('priorEligible', e.target.checked)} />My prior return covers 12 months, the taxpayer(s) and filing status are unchanged, and I was a U.S. citizen or resident for the entire prior year.</label>
        </>}
        {form.priorAvailability === 'unavailable' && <label className="flex min-h-11 items-start gap-2 rounded-lg border border-border p-3 text-sm leading-relaxed [&_input]:mt-1"><input type="checkbox" checked={form.priorExceptionReviewed} onChange={e => change('priorExceptionReviewed', e.target.checked)} />I reviewed the no-prior-tax exception and it does not exempt me. A full 12-month prior year with no tax liability and full-year U.S. citizenship/residency can qualify even if no return was required.</label>}
        <label className="flex min-h-11 items-start gap-2 rounded-lg border border-border p-3 text-sm leading-relaxed [&_input]:mt-1"><input type="checkbox" checked={form.reviewed} onChange={e => change('reviewed', e.target.checked)} />I reviewed tax adjustments, refundable credits and full-year withholding using the 1040-ES instructions. These are not just a balance due or payments made.</label>
        <label className="flex min-h-11 items-start gap-2 rounded-lg border border-border p-3 text-sm leading-relaxed [&_input]:mt-1"><input type="checkbox" checked={form.regular} onChange={e => change('regular', e.target.checked)} />The ordinary calendar-year regular method applies, with income from the first period. I do not need annualization, farming/fishing, nonresident, fiscal-year, section 1062 or special-relief rules.</label>
