...
| # | Step | Why |
| --- | --- | --- |
| 1 | Deploy the site-fixes pull request (`cursor/gtm-site-fixes-008d`, based on `codex/cursor-latest-preview-20260924`) through the normal release process. | It adds the four posts linked below and fixes `robots.txt` and the sitemap. |
| 2 | Set `NEXT_PUBLIC_GA_MEASUREMENT_ID` in the production build environment in the same deploy. `G-1P3GNBHB9J` was the tag the site used before; confirm you own that property. | The live site loads no analytics tag, so no view can be counted today. The tag loads only on public pages. |
