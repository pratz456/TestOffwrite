...
- Application commit: `66301e0162af5e2ad654611968f0d72e8576ca52` on `codex/staging-readiness`.
- Build checked by the isolated HTTP suite: `-XoCbcGUamlmGV97KJ3Yx`.
...
|---|---:|---|
| Application tests | 818 passed | Regression coverage of the integrated candidate; provider transport is mocked where applicable |
...
| Deployed Firebase smoke suite | 17 passed | Real synthetic users, sessions, profile/record persistence, receipt ownership, export gating and staging metadata |
| Deployed plan transitions | 5 passed | One persisted income record remains unchanged/readable through free, trial, expired, paid and past-due states; CSV gates correctly |
...
The HTTP suite covers free, active/expired trial, active paid, cancel-at-period-end, canceled, past-due, expired and malformed subscription snapshots. Read endpoints remain available across plan states; protected reports/CSV unlock only when persisted entitlement permits them. Requests cannot unlock features by submitting their own plan flags. These fixture checks do not validate real Stripe checkout or webhook delivery.
...
- **Dashboard estimates (P1):** `components/dashboard-screen.tsx` subtracts every debit from every credit without year, business-classification or pending-status checks. A $42.50 personal expense and $25 unreviewed expense produced a displayed “Schedule C Profit” of −$68. The same local value feeds quarterly estimates. Replace these tax cards with the shared server snapshot and preserve review/error states; until then, these estimates are a release gate. This finding is separate from the server JSON/PDF pathway.
- **New-entry default date (P2):** both manual-entry components initialize today from UTC `toISOString()`. At Pacific evening on September 15, the form defaulted to September 16. Use local calendar components for defaults/reset and test December 31 boundaries. Saved-date display fixes are verified, but this separate default remains unfixed.
...
The final application batch deployed successfully on September 15 at 11:08 p.m. Pacific (September 16, 06:08 UTC):
