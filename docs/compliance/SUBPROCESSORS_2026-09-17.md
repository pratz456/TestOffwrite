...
- Purpose: (a) transaction analysis suggestions (`lib/ai/analyzeTransaction.ts`, chat completions, model `gpt-4.1-mini` per `lib/openai/client.ts`); (b) extraction of W-2, 1099, and platform-summary images (`app/api/tax/import-document/route.ts`, `gpt-4o` vision); (c) extraction of bank/credit-card statement and receipt images (`app/api/tax/import-bank-statement/route.ts`, vision); (d) voice command parsing (`app/api/ai/parse-voice-command`, `gpt-4o-mini`: the transcribed spoken expense text); (e) general tax guidance chat (`app/api/ai/tax-assistant`, `gpt-4o`: the user's typed question and up to 12 recent messages, no profile data read from Firestore). `lib/openai/analysis.ts` is a legacy module that no route imports; it sends nothing.
...
- Fields sent for transaction analysis (`contextData`):
  ...
  - Not sent: name, email, UID, SSN, EIN, bank account numbers, Plaid tokens. Since 2026-09-18 every string in the analysis payload passes through the shared redactor (`redactIdentifierStrings`, `lib/security/identifier-redaction.ts`): SSN/ITIN/EIN shapes with any separator, bare nine-digit runs (including after an OCR-misread colon such as `SSN.123456789`) are replaced with `[redacted-id]` while dollar amounts are preserved; `attendees`, `travel_destination`, `equipment_details`, `mileage_details`, `counterparties`, the profile purpose and prior confirmed purposes are covered by default rather than by a per-field list.
- Fields sent for document import (W-2, 1099, platform summary): the image is first read by local Tesseract OCR on WriteOff's server (`lib/ocr/document-text.ts`); SSN/ITIN/EIN-shaped numbers and bare nine-digit runs in that text are replaced with `[redacted-id]` (`lib/security/identifier-redaction.ts`, the taxpayer's own SSN keeps its last four digits for organizer matching) and only the redacted text plus the extraction prompt is sent. The employer/payer EIN is read locally and never disclosed. The entire image (base64) is sent only when OCR is unusable (PDF, confidence below 0.5, too little text) or the model returns `needs_image`, and then only when the request carries `documentImageConsent=true` and the account holds a signed, current `consents.document_import` §7216 consent; otherwise the route returns 403 `DOCUMENT_IMAGE_CONSENT_REQUIRED`. The statement/receipt importer (`import-bank-statement`) still sends whole statement images, which contain account numbers and every transaction on the page.
