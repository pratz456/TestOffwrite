...
| `SSN_ENCRYPTION_KEY` | Same re-encryption approach for the `tax_organizers` identifier fields in `lib/tax-organizer/identifiers.ts` (`taxpayerSSN`, `spouseSSN`, `bankAccount`, `bankRouting`, `ipPin`, `ein`, `dependent*SSN`; `isEncrypted` identifies ciphertext). Old key must be kept until re-encryption completes | Organizer reads fail for un-migrated documents |
