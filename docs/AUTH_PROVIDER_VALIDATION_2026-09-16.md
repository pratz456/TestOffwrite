...
Google login is now verified in staging. Firebase Admin confirms a Google-linked, email-verified account created and signed in on September 16 at 11:56:13 UTC. The protected onboarding screen loaded and a full browser reload restored the signed-in session. The earlier report missed this completed result because the popup itself was not automation-accessible. No additional OAuth configuration change was necessary. The new profile identity helper hardens late identity hydration: it retrieves the same signed-in account email and preserves typed answers. After v8 deployment, a browser reload restored the account and a screenshot confirmed the correct email visibly populated. The browser accessibility/DOM inspection omitted the disabled field value even when the screenshot showed it; the earlier apparent blank field therefore cannot establish an observed rendering defect.
