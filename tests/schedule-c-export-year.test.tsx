...
const harness = vi.hoisted(() => ({ slots: [] as any[], cursor: 0, effects: [] as (() => void)[], fetch: vi.fn(), error: vi.fn(), download: { href: '', download: '', click: vi.fn() } }));
...
vi.mock('@/lib/firebase/api-client', () => ({ makeAuthenticatedRequest: async () => new Response(JSON.stringify({ transactions: [] })) }));
...
function render() {
  harness.cursor = 0;
  const tree = ScheduleCExportScreen(props);
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { URL: { createObjectURL: () => 'blob:synthetic', revokeObjectURL: vi.fn() } });
