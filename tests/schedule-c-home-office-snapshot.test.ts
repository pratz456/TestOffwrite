...
describe('home office in the shared federal snapshot', () => {
  ...
  it('counts W-2 wages toward the §179 business-income limit and lets the resulting loss zero the home office', () => {
    const input: Partial<SnapshotInput> = { grossReceipts: [{ amount: 6000 }], w2Entries: [{ box1Wages: 60000, box3SocialSecurityWages: 60000, box5MedicareWages: 60000, box2FederalWithheld: 0 }], homeOffice: homeOffice(), assets: [asset({ section179Requested: true })] };
    // A §179-created Schedule C loss offsets wages only after the at-risk, participation and profit-motive facts are saved.
    expect(() => snapshot(input)).toThrow(expect.objectContaining({ code: 'BUSINESS_LOSS_REVIEW_REQUIRED' }));
    const result = snapshot({ ...input, organizer: { ...reviewedPersonalDeductionOrganizer(), businessLossFacts: reviewedBusinessLossFacts(2026) } });
    expect(result.scheduleC).toMatchObject({ section179BusinessIncome: 66000, depreciationDeduction: 10000, tentativeProfit: -4000, homeOfficeDeduction: 0, netProfit: -4000 });
    ...
    expect(result.income.scheduleCAllowed).toBe(-4000);
  });
