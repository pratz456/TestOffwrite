...
const mocks = vi.hoisted(() => ({ deleteQuery: vi.fn(), deleteUser: vi.fn(), deleteProfile: vi.fn(), deleteHandoffs: vi.fn() }));
...
vi.mock('@/lib/preparer/handoffs', () => ({ deletePreparerHandoffsForUser: mocks.deleteHandoffs }));
...
it('account deletion includes durable tasks using their actual owner field and never an unscoped query', async () => {
  ...
  mocks.deleteHandoffs.mockResolvedValue(undefined);
  expect(await deleteUserData('synthetic-deleted-owner')).toEqual({});
  expect(mocks.deleteHandoffs).toHaveBeenCalledExactlyOnceWith('synthetic-deleted-owner');
  ...
  expect(mocks.deleteHandoffs.mock.invocationCallOrder[0]).toBeLessThan(mocks.deleteUser.mock.invocationCallOrder[0]);
});
