...
vi.mock('@/lib/firebase/admin', () => ({
  ...
  adminDb: { collection: (name: string) => ({
    where: (field: string, operator: string, value: string) => ({ name, field, operator, value }),
...
it('account deletion includes durable tasks using their actual owner field and never an unscoped query', async () => {
  ...
  expect(mocks.deleteQuery).toHaveBeenCalledWith({ name: 'analysis_tasks', field: 'userId', operator: '==', value: 'synthetic-deleted-owner' }, 500);
  for (const name of ['analysis_jobs', 'analysis_status']) {
    for (const field of ['userId', 'user_id']) {
      expect(mocks.deleteQuery).toHaveBeenCalledWith({ name, field, operator: '==', value: 'synthetic-deleted-owner' }, 500);
    }
  }
