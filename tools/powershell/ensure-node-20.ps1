# Legacy filename retained for existing shortcuts; selects the current Node 22 runtime.
# Quick script to ensure Node 22.23.2 is active
# Run: . .\ensure-node-20.ps1

Write-Host "🔧 Ensuring Node 22.23.2 is active..." -ForegroundColor Cyan
...
if ($currentVersion -like "*22.23.2*") {
    Write-Host "✅ Node 22.23.2 is already active!" -ForegroundColor Green
} else {
    ...
    if ($finalVersion -like "*22.23.2*") {
        Write-Host "✅ Successfully switched to Node 22.23.2!" -ForegroundColor Green
    } else {
