...
The most common rule today: **$100,000 in annual sales into the state** creates an obligation, even without physical presence. Some states still add or substitute a 200-transaction test, several have dropped transaction counts entirely, and a few large states (California, New York, Texas) use a $500,000 threshold. Thresholds change frequently, so confirm the current rule with each state's revenue department.
...
*Most states' economic nexus threshold: $100,000 in annual sales, with some states also using a 200-transaction test and a few using $500,000; verify each state's current rule. No federal sales tax exists. Marketplace facilitator laws (Etsy, Amazon) shift collection obligations to platforms in most states.*
